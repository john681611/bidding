...
def test_get_winning_bid():
    ...
    assert utils.get_winning_bid(bids) == {'user_id': 5, 'amount': 12.5, 'timestamp': 13, 'price_paid': 7.5}
...
def test_get_winning_bid_same_ammount():
    ...
    assert utils.get_winning_bid(bids) == {'user_id': 5, 'amount': 12.5, 'price_paid': 12.5, 'timestamp': 13}
